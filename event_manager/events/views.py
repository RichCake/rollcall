...
class UpdateEventView(AuthorRequiredMixin, views.UpdateView):
    ...
    form_class = EventForm
    queryset = Event.objects.get_public_events().only(
        "category__name",
        "title",
        "description",
        "created",
    ...
    def get_context_data(self, **kwargs):
        ...
        context["participants"] = EventParticipants.objects.filter(
            event=self.object,
        ).select_related("user")
        return context
...
class SendRequestView(LoginRequiredMixin, views.View):
    def post(self, request):
        ...
        if form.is_valid():
            ...
            accepted_participants = EventParticipants.objects.filter(
                status=EventParticipants.StatusChoices.REQUEST_ACCEPTED,
                event=event,
            )
            if (
                event.max_participants and accepted_participants.count() >= event.max_participants
            ) or event.participants.filter(
            ...
            social = event.author.social_auth.filter(
                provider="telegram",
            ).first()
            if social:
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            PeriodicTask.objects.filter(
                name=f"Send notification to {user_id} for {event_id}",
            ).delete()
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if request.user == event.author:
                user = get_object_or_404(get_user_model(), id=user_id)
                part = EventParticipants.objects.filter(
                    user=user,
                    event=event,
                ).get()
                ...
                if social:
                    ...
                    PeriodicTask.objects.create(
                        ...
                        args=json.dumps(
                            [
                                f"Через 30 минут будет {event.title}!",
                                social.uid,
                            ],
                        ),
                    )
...
class EventsListView(views.ListView):
    ...
    def get_queryset(self):
        queryset = Event.objects.get_events_with_info(
            self.request.user,
...
class DetailEventView(LoginRequiredMixin, views.DetailView):
    ...
    context_object_name = "event"
    queryset = Event.objects.select_related("author", "category", "game").only(
        "category__name",
        "title",
        "description",
        "created",
    ...
    def get_context_data(self, **kwargs):
        ...
        participants = (
            EventParticipants.objects.select_related("user")
            .filter(event__id=self.object.id)
            .only("present", "user__username", "user__id")
        ).all()
        accepted_participants = participants.filter(
            status=EventParticipants.StatusChoices.REQUEST_ACCEPTED,
        )
        contex["participants"] = accepted_participants[:5]
        contex["part_count"] = accepted_participants.count()
        contex["is_sent_request"] = participants.filter(
...
class ControlPanelEventView(views.DetailView):
    ...
    context_object_name = "event"
    queryset = Event.objects.select_related("author", "category")
...
class EventParticipantsListView(views.ListView):
    ...
    def get_queryset(self):
        ...
        return queryset.filter(event__pk=pk)
